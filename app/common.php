...
//是否是超级权限
function is_super($uid = 0)
{
	$pid = Db::name('Admin')->where(['id' => $uid])->value('position_id');
	$group_id = Db::name('PositionGroup')->where(['pid' => $pid])->column('group_id');
	$str = implode(",", $group_id);
	$arr = explode(",", $str);
	if (in_array(1, $arr)) {
		return 1;
	}
